...
SELECT COUNT(ManagerID) AS [Number of Employees with a Manager]
FROM Employees
...
SELECT CONVERT(NVARCHAR, GETDATE(), 104) + ' ' + CONVERT(NVARCHAR, GETDATE(), 114) AS [Current Date and Time]
...
[Password] NVARCHAR(100) CHECK (LEN([Password]) >= 5) NOT NULL,
...
WHERE CONVERT(NVARCHAR(10), LastLogin, 102) = CONVERT(NVARCHAR(10), GETDATE(), 102)
GO
...
VALUES
('CSMaster', 'S726S*&Skoo', 'Alan Turing', GETDATE()),
('gravityRules', 'ajdkaf&SH', 'Isaac Newton', GETDATE()),
('imadegravityrulemore', 'AKDH897aj%', 'Albert Einstein', GETDATE()),
('anon', '12345', 'John Doe', GETDATE())
...
SELECT
LOWER(LEFT(FirstName, 3) + LastName), --using first 3 letters to avoid duplicates
...
VALUES
(GETDATE(), 'New Project', 1, 'Read project requirements'),
(GETDATE(), 'Do Research', 4, 'Google stuff'),
(GETDATE(), 'Write Code', 3, 'Implement logic'),
(GETDATE(), 'Relax', 2, 'Browse internet')

UPDATE WorkHours
SET Comments = 'Go to Stackoverflow'
WHERE Task LIKE '%Google%'

DELETE FROM WorkHours
WHERE Comments LIKE '%internet%'
...
AS INSERT INTO WorkHoursLogs(OldRecord, NewRecord, Command, EmployeeId)
VALUES('', (SELECT 'Day: ' + CAST(Date AS NVARCHAR(50)) + ' ' + ' Task: ' + Task + ' ' + 'Hours: ' + CAST([Hours] AS NVARCHAR(50)) + ' ' + Comments FROM Inserted),
'INSERT',
...
AS INSERT INTO WorkHoursLogs(OldRecord, NewRecord, Command, EmployeeId)
VALUES((SELECT 'Day: ' + CAST(Date AS NVARCHAR(50)) + ' ' + ' Task: ' + Task + ' ' + 'Hours: ' + CAST([Hours] AS NVARCHAR(50)) + ' ' + Comments FROM Deleted),
(SELECT 'Day: ' + CAST(Date AS NVARCHAR(50)) + ' ' + ' Task: ' + Task + ' ' + 'Hours: ' + CAST([Hours] AS NVARCHAR(50)) + ' ' + Comments FROM Inserted),
'UPDATE',
(SELECT EmployeeID FROM Inserted))
GO
...
INSERT INTO WorkHours
VALUES(GETDATE(), 'Document', 8, 'Document things')

DELETE FROM WorkHours
WHERE Task = 'Do Research'
